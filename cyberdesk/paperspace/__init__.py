...
def draw_rect_portrait(ctx, markers, title="",
	page_width_mm=DIN_A4_WIDTH_MM, page_height_mm=DIN_A4_HEIGHT_MM,
	page_width_points=DIN_A4_WIDTH_POINTS):
	...
	page = {"page_width_mm": page_width_mm, "page_width_points": page_width_points}

	marker_size_points = (mm_to_points(marker_size[0], **page), mm_to_points(marker_size[1], **page))
	
	draw_image(ctx, tl, (mm_to_points(tl_pos[0], **page), mm_to_points(tl_pos[1], **page)), marker_size_points)
	draw_image(ctx, tr, (mm_to_points(tr_pos[0], **page), mm_to_points(tr_pos[1], **page)), marker_size_points)
	draw_image(ctx, br, (mm_to_points(br_pos[0], **page), mm_to_points(br_pos[1], **page)), marker_size_points)
	draw_image(ctx, bl, (mm_to_points(bl_pos[0], **page), mm_to_points(bl_pos[1], **page)), marker_size_points)
	
	if title != "":
		ctx.set_font_size(15)
		extents = ctx.text_extents(title)
		ctx.move_to(
			mm_to_points(page_size[0]/2, **page) - extents.width/2,
			mm_to_points(page_size[1] - border[1]/2, **page)
		)
		ctx.show_text(title)

def draw_rect_portrait_a4(ctx, markers, title=""):
	return draw_rect_portrait(ctx, markers, title,
		page_width_mm=DIN_A4_WIDTH_MM, page_height_mm=DIN_A4_HEIGHT_MM,
		page_width_points=DIN_A4_WIDTH_POINTS)
	
def draw_rect_portrait_a5(ctx, markers, title=""):
	return draw_rect_portrait(ctx, markers, title,
		page_width_mm=DIN_A5_WIDTH_MM, page_height_mm=DIN_A5_HEIGHT_MM,
		page_width_points=DIN_A5_WIDTH_POINTS)
